...
reportForm.addEventListener('submit', async function(e) {
    ...
    const formData = {
        ...
        losses: (() => {
            ...
            if (selectedOptions.length > 0) {
                const values = selectedOptions.map(opt => opt.value);
                // Якщо є "Інше", використовуємо customLosses
                if (values.includes('Інше')) {
                    return document.getElementById('customLosses').value;
                }
                return values.join(', ');
            }
            return '';
...
function showCustomInputs(show) {
    ...
    customWrappers.forEach(wrapper => {
        // Пропускаємо wrapper для модифікацій і каналу - вони мають свою логіку
        const input = wrapper.querySelector('input');
        if (input && (input.id === 'modifications' || input.id === 'channel')) {
            return; // Не чіпаємо поля модифікацій і каналу
        }
...
function initRouter() {
    ...
    navTabs.forEach(tab => {
        tab.addEventListener('click', function(e) {
            ...
            if (page === 'archive') {
                e.preventDefault();
                alert('🚧 Функціонал архіву в розробці.\nОчікуйте у версії 2.1!');
                return;
            }
